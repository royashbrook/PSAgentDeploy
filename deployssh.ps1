...
function Publish-Job{
  ...
  if ($checksettings){
      ...
      if($job.schedule -eq "daily-9am"){ $sb = "schtasks /f /create /tn '$p' /tr 'powershell c:\jobs\$p\job.ps1' /ru system /sc daily /sd 01/01/2001 /st 09:00"}
      if($job.schedule -eq "daily-10am"){ $sb = "schtasks /f /create /tn '$p' /tr 'powershell c:\jobs\$p\job.ps1' /ru system /sc daily /sd 01/01/2001 /st 10:00"}
